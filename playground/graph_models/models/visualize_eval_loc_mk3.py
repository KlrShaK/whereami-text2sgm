...
### FILTERING MESH FOR GROUND and keeping sampled points above GROUND and within walls
"""
...
def evaluate_scene(scene_id: str,
    ...
    grid_probs = softmax_probs(counts, args.score_tau)
    ...
    if have_arrow_helpers:
        ...
        for gy_i in range(0, Ny, stride):
            for gx_i in range(0, Nx, stride):
                ...
                arrow_counts.append(float(count))
    ...
    print(f"    {'oid':<8} {'label':<22} {'d_GT':>7} {'GT':>4} {'d_pred':>8} {'pred':>5}")
    for oid in sorted(centroids):
        ...
        print(f"    {oid:<8} {label:<22} {d_gt:>6.2f}m {v_gt:>4} {d_pred:>7.2f}m {v_pred:>5}")
