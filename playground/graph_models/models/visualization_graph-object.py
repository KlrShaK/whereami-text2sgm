...
# import argparse, json, random, torch, numpy as np, open3d as o3d, sys
# from pathlib import Path
...
# def load_scene(scan_dir: Path):
#     """
#     Reads labels.instances.annotated.v2.ply under `scan_dir` and returns:
#       mesh      : legacy‐CPU open3d.geometry.TriangleMesh
#       obj2faces : dict[int→np.ndarray] mapping object‐ID→face indices
#     """
#     ply = scan_dir / "labels.instances.annotated.v2.ply"
#     if not ply.exists():
#         raise FileNotFoundError(f"Missing mesh file: {ply}")

#     # 1) read & force CPU‐legacy mesh
#     mesh = o3d.io.read_triangle_mesh(str(ply))
#     mesh.compute_vertex_normals()
#     try:
#         mesh = mesh.cpu()
...
#     # 2) pack per‐vertex RGB→24bit integer
#     vc   = np.asarray(mesh.vertex_colors)
#     vc8  = (vc * 255 + 0.5).astype(np.uint32)
#     vhex = (vc8[:,0] << 16) | (vc8[:,1] << 8) | vc8[:,2]

#     # 3) load objects.json (one level up from scan_dir)
#     objj = scan_dir.parent / "objects.json"
#     if not objj.exists():
#         raise FileNotFoundError(f"Missing metadata: {objj}")
#     scans = {s["scan"]: s for s in json.load(open(objj))["scans"]}
#     meta  = scans.get(scan_dir.name)
#     if meta is None:
#         raise KeyError(f"Scan {scan_dir.name} not in {objj}")

#     # 4) hex‐colour → object‐ID map
#     colour2oid = {
#         int(o["ply_color"].lstrip("#"),16): int(o["id"])
#         for o in meta["objects"]
#     }

#     # 5) vertex → object‐ID array
#     v_oid = np.array([ colour2oid.get(int(h),0) for h in vhex ], dtype=np.int64)

#     # 6) facewise majority vote
#     tris  = np.asarray(mesh.triangles)
#     f_oid = np.array([ np.bincount(v_oid[t]).argmax() for t in tris ],
#                      dtype=np.int64)
...
#     if not obj2faces:
#         raise RuntimeError(f"No labeled faces for {scan_dir.name}")
#     return mesh, obj2faces


# def colour_objects(mesh, obj2faces, matched, base=(0.6,0.6,0.6)):
#     """
#     Paint *every* vertex light‐grey, then for each oid in `matched`
#     color all its triangles’ vertices with a bright random RGB.
#     """
#     n_v   = len(mesh.vertices)
#     cols  = np.tile(base, (n_v,1))
#     rng   = np.random.default_rng(42)
...
#     tris = np.asarray(mesh.triangles)
#     for oid in matched:
#         if oid not in obj2faces: continue
...
# @torch.inference_mode()
# def compute_match_score(model, qg, sg, device):
#     """
#     Runs the GNN (if provided) on (qg, sg). Returns the scalar
#     matching probability m_p.  If model is None, falls back
#     to DBSCAN overlap size / dummy score.
#     """
#     if model is None:
#         # fallback: size of overlap subgraph
#         print("No GNN model; using pure-overlap matcher")
#         _, sub3d = get_matching_subgraph(qg, sg)
#         return float(len(sub3d.nodes)) if sub3d else 0.0
...
#     # forward pass → (_,_,m_p)
#     _, _, m_p = model(x_n, p_n, x_e, p_e, x_f, p_f)
#     return m_p.item()


# def visualize_match(scan_root: Path, query_key: str, scene_id: str,
#                     captions, scene_graphs, model, device):
#     """
#     Visualize a single query caption vs one scene:
#       - load the mesh for scene_id
#       - find matched object IDs
#       - colour and show
#     """
#     # 1) build SceneGraph objects
#     qg = captions[query_key]
#     sg = scene_graphs[scene_id]

#     # 2) compute which scene‐node IDs matched (DBSCAN overlap)
#     _, sub3d = get_matching_subgraph(qg, sg)
#     matched = [] if sub3d is None else list(sub3d.nodes)

#     # 3) load & colour mesh
#     mesh, obj2faces = load_scene(scan_root/scene_id)
#     vis_mesh = colour_objects(mesh, obj2faces, matched)

#     # 4) show with keyboard controls
#     print(f"\n>>> Query {query_key} vs scene {scene_id}")
#     print("Matched object IDs:", matched)
#     print(" SPACE/ENTER → close window;  q/Esc → quit")

#     vis = o3d.visualization.VisualizerWithKeyCallback()
#     vis.create_window(window_name=f"{query_key}→{scene_id}", width=1024, height=768)
#     vis.add_geometry(vis_mesh)
#     vis.register_key_callback(32,  lambda v: v.close())
#     vis.register_key_callback(257, lambda v: v.close())
#     vis.register_key_callback(81,  lambda _: sys.exit(0))
#     vis.register_key_callback(256, lambda _: sys.exit(0))
#     vis.run()
#     vis.destroy_window()
...
#     p = argparse.ArgumentParser()
#     p.add_argument("--root",      required=True,
#                    help="path to parent of 3RScan/<scan_id>/")
#     p.add_argument("--graphs",    required=True,
#                    help="folder containing 3dssg_*.pt and scanscribe_*.pt")
#     p.add_argument("--query-key", required=True,
#                    help="one ScanScribe caption key, e.g. 0958…f7_00000")
#     p.add_argument("--ckpt",      default=None,
...
#     # 1) load all 3DSSG scene graphs
#     graph_dir = Path(args.graphs)
#     raw_scenes = torch.load(graph_dir/"3dssg"/"3dssg_graphs_processed_edgelists_relationembed.pt",
#                             map_location="cpu")
#     scene_graphs = {
#       sid: SceneGraph(sid, graph_type="3dssg", graph=g,
#                       max_dist=1.0, embedding_type="word2vec",
#                       use_attributes=True)
#       for sid,g in raw_scenes.items()
#     }

#     # 2) load all ScanScribe caption‐graphs
#     caps_raw = torch.load(graph_dir/"scanscribe"/"scanscribe_text_graphs_from_image_desc_node_edge_features.pt",
#                           map_location="cpu")
#     captions = {
#       k: SceneGraph(k.split("_")[0], txt_id=None,
#                     graph=g, graph_type="scanscribe",
#                     embedding_type="word2vec", use_attributes=True)
#       for k,g in caps_raw.items()
#     }
#     if args.query_key not in captions:
#         raise KeyError(f"{args.query_key} not found among {len(captions)} captions")

#     # 3) load or skip BigGNN
#     model = None
#     if args.ckpt:
#         ckpt = torch.load(args.ckpt, map_location=args.device)
#         model = BigGNN(args.N, args.heads).to(args.device)
...
#     # 4) score every scene for the one query
#     scores = {}
#     for sid, sg in scene_graphs.items():
#         scores[sid] = compute_match_score(model, captions[args.query_key],
#                                           sg, args.device)

#     # 5) pick best & worst
#     best_sid  = max(scores, key=lambda s: scores[s])
#     worst_sid = min(scores, key=lambda s: scores[s])
#     print(f"\n→ Best match:  {best_sid}  (score={scores[best_sid]:.4f})")
#     print(f"→ Worst match: {worst_sid} (score={scores[worst_sid]:.4f})")

#     # 6) visualize both
#     visualize_match(Path(args.root), args.query_key, best_sid,
#                     captions, scene_graphs, model, args.device)
#     visualize_match(Path(args.root), args.query_key, worst_sid,
#                     captions, scene_graphs, model, args.device)
